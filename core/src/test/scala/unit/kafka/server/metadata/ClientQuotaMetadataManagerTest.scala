...
class ClientQuotaMetadataManagerTest {
  ...
  def testDescribeClientQuotasInvalidFilterCombination(): Unit = {
    ...
    def describeClientQuotas(components: Seq[ClientQuotaFilterComponent]): Executable = {
      () => cache.describeClientQuotas(components, strict=false)
    }

    assertThrows(classOf[InvalidRequestException],
      describeClientQuotas(Seq(ipFilterComponent, userFilterComponent)),
      expectedExceptionMessage)
    assertThrows(classOf[InvalidRequestException],
      describeClientQuotas(Seq(ipFilterComponent, clientIdFilterComponent)),
      expectedExceptionMessage)
    assertThrows(classOf[InvalidRequestException],
      describeClientQuotas(Seq(ipFilterComponent, ipFilterComponent)),
      expectedExceptionMessage)
    assertThrows(classOf[InvalidRequestException],
      describeClientQuotas(Seq(userFilterComponent, userFilterComponent)),
      expectedExceptionMessage)
  }
