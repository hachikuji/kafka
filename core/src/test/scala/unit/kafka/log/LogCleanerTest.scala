...
class LogCleanerTest extends JUnitSuite {
  ...
  def testCleaningWithUncleanableSection(): Unit = {
    ...
    def distinctValuesBySegment = log.logSegments.map(s => s.log.asScala.map(m => TestUtils.readString(m.record.value)).toSet.size).toSeq
  ...
  def testLogToClean(): Unit = {
    ...
    val messageSet = TestUtils.singleRecordSet(value = Array.fill[Byte](50)(0), key = 1.toString.getBytes)
    for (_ <- 0 until 6)
  ...
  def keysInLog(log: Log): Iterable[Int] =
    log.logSegments.flatMap(s => s.log.asScala.filter(!_.record.hasNullValue).filter(_.record.hasKey).map(m => TestUtils.readString(m.record.key).toInt))

  /* extract all the offsets from a log */
  def offsetsInLog(log: Log): Iterable[Long] =
    log.logSegments.flatMap(s => s.log.asScala.filter(!_.record.hasNullValue).filter(_.record.hasKey).map(m => m.offset))

  def unkeyedMessageCountInLog(log: Log) =
    log.logSegments.map(s => s.log.asScala.filter(!_.record.hasNullValue).count(m => !m.record.hasKey)).sum
  ...
  def testSegmentGrouping(): Unit = {
    ...
    while(log.numberOfSegments < 10) {
      log.append(TestUtils.singleRecordSet(value = "hello".getBytes, key = "hello".getBytes))
      i += 1
    }
  ...
  def testCleanCorruptMessageSet() {
    ...
    for (segment <- log.logSegments; shallowLogEntry <- segment.log.asScala; deepLogEntry <- shallowLogEntry.asScala) {
  ...
  def deleteMessage(key: Int) = record(key, null)
