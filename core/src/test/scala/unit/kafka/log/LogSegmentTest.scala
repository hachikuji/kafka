...
class LogSegmentTest {
  ...
  def testMaxOffset() {
    ...
    def validate(offset: Long) =
      assertEquals(ms.asScala.filter(_.offset == offset).toList,
                   seg.read(startOffset = offset, maxSize = 1024, maxOffset = Some(offset+1)).logBuffer.asScala.toList)
    validate(50)
    validate(51)
    validate(52)
  }
  ...
  def testReadFromGap() {
    ...
    assertEquals(ms2.asScala.toList, read.logBuffer.asScala.toList)
  }
  ...
  def testTruncate() {
    ...
    for (_ <- 0 until 30) {
      ...
      assertEquals(List(ms1.asScala.head, ms2.asScala.head), read.logBuffer.asScala.toList)
      // now truncate off the last message
      seg.truncateTo(offset + 1)
      val read2 = seg.read(offset, None, 10000)
      assertEquals(1, read2.logBuffer.asScala.size)
      assertEquals(ms1.asScala.head, read2.logBuffer.asScala.head)
      offset += 1
    }
  }
  ...
  def testRecoveryFixesCorruptIndex() {
    ...
    seg.recover(64*1024)
    for(i <- 0 until 100)
      assertEquals(i, seg.read(i, Some(i + 1), 1024).logBuffer.asScala.head.offset)
  }
  ...
  def testCreateWithInitFileSizeClearShutdown() {
    ...
    assertEquals(ms2.asScala.toList, readAgain.logBuffer.asScala.toList)
