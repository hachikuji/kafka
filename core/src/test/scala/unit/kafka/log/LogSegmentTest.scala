...
class LogSegmentTest {
  ...
  def createSegment(offset: Long, indexIntervalBytes: Int = 10): LogSegment = {
    ...
    val ms = FileLogBuffer.open(msFile)
  ...
  def logBuffer(offset: Long, records: String*): MemoryLogBuffer = {
    MemoryLogBuffer.withRecords(offset, records.map(s => Record.create(Record.MAGIC_VALUE_V1, offset * 10, s.getBytes)):_*)
  }
  ...
  def testMaxOffset() {
    ...
    def validate(offset: Long) =
      assertEquals(ms.deepEntries.asScala.filter(_.offset == offset).toList,
                   seg.read(startOffset = offset, maxSize = 1024, maxOffset = Some(offset+1)).logBuffer.deepEntries.asScala.toList)
    validate(50)
    validate(51)
    validate(52)
  }
  ...
  def testReadAfterLast() {
    val seg = createSegment(40)
    val ms = logBuffer(50, "hello", "there")
    seg.append(50, Record.NO_TIMESTAMP, -1L, ms)
  ...
  def testReadFromGap() {
    ...
    assertEquals(ms2.deepEntries.asScala.toList, read.logBuffer.deepEntries.asScala.toList)
  }
  ...
  def testTruncate() {
    ...
    var offset = 40
    for (_ <- 0 until 30) {
      val ms1 = logBuffer(offset, "hello")
      seg.append(offset, Record.NO_TIMESTAMP, -1L, ms1)
      val ms2 = logBuffer(offset + 1, "hello")
      seg.append(offset + 1, Record.NO_TIMESTAMP, -1L, ms2)
      // check that we can read back both messages
      val read = seg.read(offset, None, 10000)
      assertEquals(List(ms1.deepEntries.next(), ms2.deepEntries.next()), read.logBuffer.deepEntries.asScala.toList)
      // now truncate off the last message
      seg.truncateTo(offset + 1)
      val read2 = seg.read(offset, None, 10000)
      assertEquals(1, read2.logBuffer.deepEntries.asScala.size)
      assertEquals(ms1.deepEntries.next(), read2.logBuffer.deepEntries.next())
      offset += 1
    }
  }
  ...
  def testReloadLargestTimestampAfterTruncation() {
    val numMessages = 30
    val seg = createSegment(40, 2 * logBuffer(0, "hello").sizeInBytes - 1)
    var offset = 40
    for (_ <- 0 until numMessages) {
      seg.append(offset, offset, offset, logBuffer(offset, "hello"))
      offset += 1
    }
  ...
  def testTruncateFull() {
    ...
    val seg = createSegment(40)
    seg.append(40, Record.NO_TIMESTAMP, -1L, logBuffer(40, "hello", "there"))
    seg.truncateTo(0)
  ...
  def testFindOffsetByTimestamp() {
    val messageSize = logBuffer(0, s"msg00").sizeInBytes
    val seg = createSegment(40, messageSize * 2 - 1)
    // Produce some messages
    for (i <- 40 until 50)
      seg.append(i, i * 10, i, logBuffer(i, s"msg$i"))
  ...
  def testNextOffsetCalculation() {
    ...
    seg.append(50, Record.NO_TIMESTAMP, -1L, logBuffer(50, "hello", "there", "you"))
  ...
  def testRecoveryFixesCorruptIndex() {
    ...
    for(i <- 0 until 100)
      seg.append(i, Record.NO_TIMESTAMP, -1L, logBuffer(i, i.toString))
    val indexFile = seg.index.file
    TestUtils.writeNonsenseToFile(indexFile, 5, indexFile.length.toInt)
    seg.recover(64*1024)
    for(i <- 0 until 100)
      assertEquals(i, seg.read(i, Some(i + 1), 1024).logBuffer.deepEntries.next().offset)
  }
  ...
  def testRecoveryFixesCorruptTimeIndex() {
    val seg = createSegment(0)
    for(i <- 0 until 100)
      seg.append(i, i * 10, i, logBuffer(i, i.toString))
  ...
  def testCreateWithInitFileSizeClearShutdown() {
    ...
    assertEquals(ms2.deepEntries.asScala.toList, readAgain.logBuffer.deepEntries.asScala.toList)
