...
class LogSegmentTest {
  ...
  def records(offset: Long, records: String*): MemoryLogBuffer = {
  ...
  def testReadBeforeFirstOffset() {
    ...
    val read = seg.read(startOffset = 41, maxSize = 300, maxOffset = None).logEntries
    assertEquals(ms.asScala.toList, read.asScala.toList)
  }
  ...
  def testMaxOffset() {
    ...
    def validate(offset: Long) =
      assertEquals(ms.asScala.filter(_.offset == offset).toList,
                   seg.read(startOffset = offset, maxSize = 1024, maxOffset = Some(offset+1)).logEntries.asScala.toList)
    validate(50)
    validate(51)
    validate(52)
  }
  ...
  def testReadFromGap() {
    ...
    assertEquals(ms2.asScala.toList, read.logEntries.asScala.toList)
  }
  ...
  def testTruncate() {
    ...
    for (_ <- 0 until 30) {
      ...
      assertEquals(List(ms1.asScala.head, ms2.asScala.head), read.logEntries.asScala.toList)
      // now truncate off the last message
      seg.truncateTo(offset + 1)
      val read2 = seg.read(offset, None, 10000)
      assertEquals(1, read2.logEntries.asScala.size)
      assertEquals(ms1.asScala.head, read2.logEntries.asScala.head)
      offset += 1
    }
  }
  ...
  def testRecoveryFixesCorruptIndex() {
    ...
    seg.recover(64*1024)
    for(i <- 0 until 100)
      assertEquals(i, seg.read(i, Some(i + 1), 1024).logEntries.asScala.head.offset)
  }
  ...
  def testRecoveryWithCorruptMessage() {
    ...
    for (_ <- 0 until 10) {
      ...
      seg.recover(64*1024)
      assertEquals("Should have truncated off bad messages.", (0 until offsetToBeginCorruption).toList, seg.log.asScala.map(_.offset).toList)
      seg.delete()
    }
  }
  ...
  def testCreateWithInitFileSizeClearShutdown() {
    ...
    assertEquals(ms2.asScala.toList, readAgain.logEntries.asScala.toList)
