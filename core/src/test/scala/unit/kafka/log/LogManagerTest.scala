...
class LogManagerTest {
  ...
  def testCleanupExpiredSegments() {
    ...
    assertEquals("Should get empty fetch off new log.", 0, log.read(offset+1, 1024).logBuffer.sizeInBytes)
  ...
  def testDoesntCleanLogsWithCompactDeletePolicy() {
    ...
    var offset = 0L
    for (_ <- 0 until 200) {
      val set = TestUtils.singleRecordSet("test".getBytes(), key="test".getBytes())
      val info = log.append(set)
      offset = info.lastOffset
    }
