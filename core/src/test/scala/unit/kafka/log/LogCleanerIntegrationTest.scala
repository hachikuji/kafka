...
class LogCleanerIntegrationTest(compressionCodec: String) {
  ...
  private def createLargeSingleMessageSet(key: Int, messageFormatVersion: Byte): (String, MemoryLogBuffer) = {
    ...
    val messageSet = TestUtils.singleRecordSet(value = value.getBytes, codec = codec, key = key.toString.getBytes,
  ...
  private def readFromLog(log: Log): Iterable[(Int, String, Long)] = {
    ...
    for (segment <- log.logSegments; shallowLogEntry <- segment.log.asScala; deepLogEntry <- shallowLogEntry.asScala) yield {
  ...
  private def writeDups(numKeys: Int, numDups: Int, log: Log, codec: CompressionType,
    ...
    for(_ <- 0 until numDups; key <- startKey until (startKey + numKeys)) yield {
      ...
      val appendInfo = log.append(TestUtils.singleRecordSet(value = value.toString.getBytes, codec = codec,
  ...
  private def writeDupsSingleMessageSet(numKeys: Int, numDups: Int, log: Log, codec: CompressionType,
    ...
    val records = MemoryLogBuffer.withRecords(codec, messages: _*)
