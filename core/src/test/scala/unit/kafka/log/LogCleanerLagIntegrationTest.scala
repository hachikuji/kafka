...
class LogCleanerLagIntegrationTest(compressionCodecName: String) extends Logging {
  ...
  private def readFromLog(log: Log): Iterable[(Int, Int)] = {
    ...
    for (segment <- log.logSegments; shallowLogEntry <- segment.log.asScala; logEntry <- shallowLogEntry.asScala) yield {
  ...
  private def writeDups(numKeys: Int, numDups: Int, log: Log, codec: CompressionType, timestamp: Long): Seq[(Int, Int)] = {
    for (_ <- 0 until numDups; key <- 0 until numKeys) yield {
      val count = counter
      log.append(TestUtils.singletonLogBuffer(value = counter.toString.getBytes, codec = codec, key = key.toString.getBytes, timestamp = timestamp), assignOffsets = true)
      counter += 1
      (key, count)
    }
  }
