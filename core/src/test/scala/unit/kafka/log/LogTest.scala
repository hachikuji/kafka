...
class LogTest extends JUnitSuite {
  ...
  def testTimeBasedLogRoll() {
    val set = TestUtils.singleRecordSet("test".getBytes)
    ...
    val setWithTimestamp =
      TestUtils.singleRecordSet(value = "test".getBytes, timestamp = time.milliseconds + log.config.segmentMs + 1)
    ...
    val setWithExpiredTimestamp = TestUtils.singleRecordSet(value = "test".getBytes, timestamp = time.milliseconds)
  ...
  def testLoadEmptyLog() {
    ...
    log.append(TestUtils.singleRecordSet("test".getBytes))
  }
  ...
  def testAppendAndReadWithSequentialOffsets() {
    ...
    for(i <- records.indices) {
      val read = log.read(i, 100, Some(i+1)).logBuffer.asScala.head
    ...
    assertEquals("Reading beyond the last message returns nothing.", 0, log.read(records.length, 100, None).logBuffer.asScala.size)
  }
  ...
  def testAppendAndReadWithNonSequentialOffsets() {
    ...
    for(i <- 50 until messageIds.max) {
      ...
      val read = log.read(i, 100, None).logBuffer.asScala.head
  ...
  def testReadAtLogGap() {
    ...
    assertEquals("A read should now return the last message in the log", log.logEndOffset - 1, log.read(1, 200, None).logBuffer.asScala.head.offset)
  }
  ...
  def testReadWithMinMessage() {
    ...
    for (i <- 50 until messageIds.max) {
      ...
      ).map(_.logBuffer.asScala.head)
      reads.foreach { read =>
        assertEquals("Offset read should match message id.", messageIds(idx), read.offset)
        assertEquals("Message should match appended.", records(idx), read.record)
      }

      assertEquals(Seq.empty, log.read(i, 1, Some(1), minOneMessage = true).logBuffer.asScala.toIndexedSeq)
    }
  ...
  def testLogRolls() {
    ...
    val log = new Log(logDir, LogConfig(logProps), recoveryPoint = 0L, time.scheduler, time = time)
    val numMessages = 100
    val messageSets = (0 until numMessages).map(i => TestUtils.singleRecordSet(i.toString.getBytes))
    ...
    for(i <- 0 until numMessages) {
      val messages = log.read(offset, 1024*1024).logBuffer.asScala
      assertEquals("Offsets not equal", offset, messages.head.offset)
      assertEquals("Messages not equal at offset " + offset, messageSets(i).asScala.head.record,
        messages.head.record.convert(messageSets(i).asScala.head.record.magic))
  ...
  def testCompressedMessages() {
    ...
    def read(offset: Int) = log.read(offset, 4096).logBuffer.iterator(false)
  ...
  def testThatGarbageCollectingSegmentsDoesntChangeOffset() {
    for(messagesToAppend <- List(0, 1, 25)) {
      ...
      for(i <- 0 until messagesToAppend)
        log.append(TestUtils.singleRecordSet(value = i.toString.getBytes, timestamp = time.milliseconds - 10))
      ...
      assertEquals("Should still be able to append and should get the logEndOffset assigned to the new append",
                   currOffset,
                   log.append(TestUtils.singleRecordSet("hello".getBytes)).firstOffset)
  ...
  def testLogRecoversToCorrectOffset() {
    ...
    for(i <- 0 until numMessages)
      log.append(TestUtils.singleRecordSet(value = TestUtils.randomBytes(messageSize),
  ...
  def testIndexRebuild() {
    ...
    var log = new Log(logDir, config, recoveryPoint = 0L, time.scheduler, time)
    for(i <- 0 until numMessages)
      log.append(TestUtils.singleRecordSet(value = TestUtils.randomBytes(10), timestamp = time.milliseconds + i * 10))
    ...
    for(i <- 0 until numMessages) {
      assertEquals(i, log.read(i, 100, None).logBuffer.asScala.head.offset)
      if (i == 0)
  ...
  def testIndexResizingAtTruncation() {
    val setSize = TestUtils.singleRecordSet(value = "test".getBytes).sizeInBytes
    val msgPerSeg = 10
    val segmentSize = msgPerSeg * setSize  // each segment will be 10 messages
    val logProps = new Properties()
    logProps.put(LogConfig.SegmentBytesProp, segmentSize: java.lang.Integer)
    ...
    for (i<- 1 to msgPerSeg)
      log.append(TestUtils.singleRecordSet(value = "test".getBytes, timestamp = time.milliseconds + i))
    assertEquals("There should be exactly 1 segment.", 1, log.numberOfSegments)
  ...
  def testAppendMessageWithNullPayload() {
    ...
    val messageSet = log.read(0, 4096, None).logBuffer.asScala
    assertEquals(0, messageSet.head.offset)
    assertTrue("Message payload should be null.", messageSet.head.record.hasNullValue)
  }
  ...
  def testCorruptLog() {
    ...
    for (_ <- 0 until 50) {
      ...
      for (_ <- 0 until numMessages)
        log.append(set)
      val messages = log.logSegments.flatMap(_.log.iterator.asScala.toList)
      log.close()
      ...
      assertEquals("Messages in the log after recovery should be the same.", messages, log.logSegments.flatMap(_.log.iterator.asScala.toList))
      Utils.delete(logDir)
    }
  }
  ...
  def shouldDeleteTimeBasedSegmentsReadyToBeDeleted() {
    val set = TestUtils.singleRecordSet("test".getBytes, timestamp = 10)
  ...
  def shouldNotDeleteTimeBasedSegmentsWhenNoneReadyToBeDeleted() {
    val set = TestUtils.singleRecordSet("test".getBytes, timestamp = time.milliseconds)
  ...
  def shouldNotDeleteSegmentsWhenPolicyDoesNotIncludeDelete() {
    val set = TestUtils.singleRecordSet("test".getBytes, key = "test".getBytes(), timestamp = 10L)
