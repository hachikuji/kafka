...
object DumpLogSegments {
  ...
  private def dumpIndex(file: File,
    ...
    val messageSet = FileLogBuffer.open(logFile, false)
    ...
    for(i <- 0 until index.entries) {
      ...
      val records = messageSet.read(entry.position, maxMessageSize)
      val logEntry = getIterator(records.iterator.next, isDeepIteration = true).next()
  ...
  private def dumpTimeIndex(file: File,
    ...
    for(i <- 0 until timeIndex.entries) {
      ...
      val partialFileMessageSet = messageSet.read(position, Int.MaxValue)
      val shallowEntries = partialFileMessageSet.asScala
      ...
      if (maybeLogEntry.isEmpty || maybeLogEntry.get.offset != entry.offset + timeIndex.baseOffset) {
        timeIndexDumpErrors.recordShallowOffsetNotFound(file, entry.offset + timeIndex.baseOffset,
          {if (maybeLogEntry.isDefined) maybeLogEntry.get.offset else -1})
      } else {
        val deepIter = getIterator(maybeLogEntry.get, isDeepIteration = true)
        for (messageAndOffset <- deepIter)
          maxTimestamp = math.max(maxTimestamp, messageAndOffset.record.timestamp)
  ...
  private def dumpLog(file: File,
    ...
    var validBytes = 0L
    var lastOffset = -1l
    val shallowIterator = messageSet.iterator(maxMessageSize).asScala
