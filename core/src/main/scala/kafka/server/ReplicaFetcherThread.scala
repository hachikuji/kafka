...
class ReplicaFetcherThread(name: String,
  ...
  def processPartitionData(topicPartition: TopicPartition, fetchOffset: Long, partitionData: PartitionData) {
    try {
      ...
      val records = partitionData.toLogBuffer

      maybeWarnIfMessageOversized(records, topicPartition)
  ...
  def maybeWarnIfMessageOversized(logBuffer: MemoryLogBuffer, topicPartition: TopicPartition): Unit = {
    // oversized messages don't cause replication to fail from fetch request version 3 (KIP-74)
    if (fetchRequestVersion <= 2 && logBuffer.sizeInBytes > 0 && logBuffer.validBytes <= 0)
  ...
  private def sendRequest(apiKey: ApiKeys, apiVersion: Option[Short], request: AbstractRequest): ClientResponse = {
    ...
    try {
      if (!networkClient.blockingReady(sourceNode, socketTimeout)(time))
        ...
        val send = request.toSend(sourceBroker.id.toString, header)
        val clientRequest = new ClientRequest(time.milliseconds(), true, header, request, send, null)
...
object ReplicaFetcherThread {
  ...
  private[server] class PartitionData(val underlying: FetchResponse.PartitionData) extends AbstractFetcherThread.PartitionData {
    ...
    def toLogBuffer: MemoryLogBuffer = {
      underlying.records.asInstanceOf[MemoryLogBuffer]
    }
