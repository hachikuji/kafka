...
import org.apache.kafka.common.metrics.{Metrics, MetricsReporter, _}
...
) extends KafkaProcess with Logging with KafkaMetricsGroup {
  ...
  override def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        metrics = KafkaProcess.initializeMetrics(config, time, clusterId)
