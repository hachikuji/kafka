...
private[kafka] object LogValidator {
   ...
   */
  private[kafka] def validateMessagesAndAssignOffsets(records: MemoryLogBuffer,
    ...
    if (sourceCodec == NoCompressionCodec && targetCodec == NoCompressionCodec) {
      ...
      records.iterator(false, true).asScala.foreach { logEntry =>
      ...
      if (!inPlaceAssignment) {
        ...
        ValidationAndOffsetAssignResult(
          validatedRecords = builder.records,
        ...
        val mutableEntry = new MutableLogEntry(records.buffer)
  ...
  private def convertAndAssignOffsetsNonCompressed(records: MemoryLogBuffer,
                                                   offsetCounter: LongRef,
    ...
    val sizeInBytesAfterConversion = records.validBytes + records.iterator(true).asScala.map { logEntry =>
    ...
    records.iterator(true).asScala.foreach { logEntry =>
      val record = logEntry.record
      validateKey(record, compactedTopic)
  ...
  private def assignOffsetsNonCompressed(records: MemoryLogBuffer,
                                         offsetCounter: LongRef,
                                         now: Long,
                                         compactedTopic: Boolean,
                                         timestampType: TimestampType,
                                         timestampDiffMaxMs: Long): ValidationAndOffsetAssignResult = {
    val updater = MemoryLogBuffer.inPlaceUpdater(records.buffer)
  ...
  case class ValidationAndOffsetAssignResult(validatedRecords: MemoryLogBuffer,
                                             maxTimestamp: Long,
