...
public class RecordAccumulatorTest {
    ...
    public void testFull() throws Exception {
        ...
        int batchSize = 1024;
        RecordAccumulator accum = new RecordAccumulator(batchSize, 10 * batchSize, CompressionType.NONE, 10L, 100, metrics, time);
    ...
    public void testAppendLarge() throws Exception {
        int batchSize = 512;
        RecordAccumulator accum = new RecordAccumulator(batchSize, 10 * 1024, CompressionType.NONE, 0L, 100, metrics, time);
    ...
    public void testLinger() throws Exception {
        long lingerMs = 10L;
        RecordAccumulator accum = new RecordAccumulator(1024, 10 * 1024, CompressionType.NONE, lingerMs, 100, metrics, time);
        accum.append(tp1, 0L, key, value, null, maxBlockTimeMs);
    ...
    public void testPartialDrain() throws Exception {
        RecordAccumulator accum = new RecordAccumulator(1024, 10 * 1024, CompressionType.NONE, 10L, 100, metrics, time);
    ...
    public void testStressfulSituation() throws Exception {
        ...
        final RecordAccumulator accum = new RecordAccumulator(1024, 10 * 1024, CompressionType.NONE, 0L, 100, metrics, time);
    ...
    public void testRetryBackoff() throws Exception {
        long lingerMs = Integer.MAX_VALUE / 4;
        int retryBackoffMs = Integer.MAX_VALUE / 2;
    ...
    public void testFlush() throws Exception {
        ...
        final RecordAccumulator accum = new RecordAccumulator(4 * 1024, 64 * 1024, CompressionType.NONE, lingerMs, 100, metrics, time);
        for (int i = 0; i < 100; i++)
    ...
    public void testAwaitFlushComplete() throws Exception {
        RecordAccumulator accum = new RecordAccumulator(4 * 1024, 64 * 1024, CompressionType.NONE, Long.MAX_VALUE, 100, metrics, time);
    ...
    public void testMutedPartitions() throws InterruptedException {
        long now = time.milliseconds();
        RecordAccumulator accum = new RecordAccumulator(1024, 10 * 1024, CompressionType.NONE, 10, 100, metrics, time);
        int appends = 1024 / msgSize;
        for (int i = 0; i < appends; i++) {
            accum.append(tp1, 0L, key, value, null, maxBlockTimeMs);
            assertEquals("No partitions should be ready.", 0, accum.ready(cluster, now).readyNodes.size());
