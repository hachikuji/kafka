...
abstract class FileLogInputStream<T extends LogEntry> implements LogInputStream<T> {
    ...
    private static class LazyFileLogInputStream extends org.apache.kafka.common.record.FileLogInputStream<FileChannelLogEntry> {
    ...
    private static class EagerFileLogInputStream extends org.apache.kafka.common.record.FileLogInputStream<LogEntry> {
        ...
        protected LogEntry nextEntry(long offset, long position, int size) throws IOException {
            ...
            if (recordBuffer.hasRemaining())
                return null;
            recordBuffer.rewind();
            return new LogEntry.SimpleLogEntry(offset, new Record(recordBuffer));
        }
