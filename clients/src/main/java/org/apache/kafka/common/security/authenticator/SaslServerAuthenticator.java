...
public class SaslServerAuthenticator implements Authenticator {
    ...
    private String handleHandshakeRequest(RequestContext context, SaslHandshakeRequest handshakeRequest) throws IOException, UnsupportedSaslMechanismException {
        ...
        if (version >= 1)
            this.enableKafkaSaslAuthenticateHeaders();
    ...
    private class ReauthInfo {
        ...
        private long calcCompletionTimesAndReturnSessionLifetimeMs() {
            ...
            if (credentialExpirationMs != null) {
                if (sessionExpirationTimeNanos != null)
                    LOG.debug("Authentication complete; session max lifetime from broker config={} ms, " +
                                    "credential expiration={} ({} ms); session expiration = {} ({} ms), " +
                                    "sending {} ms to client",
                            connectionsMaxReauthMs,
                            new Date(credentialExpirationMs),
                            credentialExpirationMs - authenticationEndMs,
                            new Date(authenticationEndMs + retvalSessionLifetimeMs),
                            retvalSessionLifetimeMs,
                            retvalSessionLifetimeMs);
                else
                    LOG.debug(
                            "Authentication complete; session max lifetime from broker config={} ms, " +
                                    "credential expiration={} ({} ms); no session expiration, " +
                                    "sending 0 ms to client",
                            connectionsMaxReauthMs,
                            new Date(credentialExpirationMs),
                            credentialExpirationMs - authenticationEndMs);
            } else {
                if (sessionExpirationTimeNanos != null)
                    LOG.debug("Authentication complete; session max lifetime from broker config={} ms, " +
                                    "no credential expiration; session expiration = {} ({} ms), " +
                                    "sending {} ms to client",
                    ...
                    LOG.debug("Authentication complete; session max lifetime from broker config={} ms, " +
                                    "no credential expiration; no session expiration, " +
                                    "sending 0 ms to client",
                            connectionsMaxReauthMs);
            }
        ...
        private Long reauthenticationLatencyMs() {
            if (!reauthenticating())
                return null;
