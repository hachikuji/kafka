...
public class EosLogRecord implements LogRecord {
    ...
    public static long writeTo(DataOutputStream out,
        ...
        if (key != null) {
            ...
            Utils.writeUnsignedVarInt(keySize, out);
    ...
    public static long writeTo(ByteBuffer out,
                               ...
                               ByteBuffer value) {
        try {
            return writeTo(new ByteBufferOutputStream(out), offsetDelta, timestampDelta, key, value);
        } catch (IOException e) {
    ...
    private static EosLogRecord readFrom(ByteBuffer buffer,
        ...
        if (hasKey(attributes)) {
            int keySizeInBytes = Utils.readUnsignedVarInt(buffer);
            key = buffer.slice();
    ...
    private static int sizeOfBodyInBytes(int offsetDelta,
        ...
        if (key != null) {
            int keySize = key.remaining();
            size += Utils.bytesForUnsignedVarIntEncoding(keySize);
            size += keySize;
        }
