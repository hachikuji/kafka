...
public class EosLogEntry extends AbstractLogEntry implements LogEntry.MutableLogEntry {
    ...
    public long lastOffset() {
        return baseOffset() + Utils.readUnsignedInt(buffer, LAST_OFFSET_DELTA_OFFSET);
    }
    ...
    public int lastSequence() {
        // FIXME: cast to int
        return baseSequence() + (int) Utils.readUnsignedInt(buffer, LAST_OFFSET_DELTA_OFFSET);
    }
    ...
    private Iterator<LogRecord> compressedIterator() {
        ...
        DataInputStream stream = MemoryRecordsBuilder.wrapForInput(new ByteBufferInputStream(buffer), compressionType(), magic());
