...
public class KafkaRaftClient implements RaftClient {
    ...
    private FetchResponseData buildFetchResponse(
        ...
        return singletonFetchResponse(log.topicPartition(), Errors.NONE, partitionData -> {
            ...
            nextFetchOffsetAndEpoch.ifPresent(header::setNextOffsetAndEpoch);
    ...
    private CompletableFuture<FetchResponseData> handleFetchRequest(
        ...
        if (partitionResponse.partitionHeader().errorCode() != Errors.NONE.code()
    ...
    private boolean handleFetchResponse(
        ...
        FetchResponseData.LeaderIdAndEpoch currentLeaderIdAndEpoch = partitionHeader.currentLeader();
        ...
        Errors error = Errors.forCode(partitionHeader.errorCode());
        ...
        if (error == Errors.NONE) {
            FetchResponseData.OffsetAndEpoch nextOffsetAndEpoch = partitionHeader.nextOffsetAndEpoch();
            if (nextOffsetAndEpoch.nextFetchOffset() > 0) {
                ...
                OptionalLong highWatermark = partitionHeader.highWatermark() < 0 ?
                    OptionalLong.empty() : OptionalLong.of(partitionHeader.highWatermark());
